...
class SmsContentTest < ActiveSupport::TestCase
  ...
  test "booking enquiry with an integer cost" do
    ...
    assert_equal "VarsityCleaners: job offer in E1W. 05 April 12:00 - 16:00 with own cleaning stuff. Will pay 48 pounds. Accept? Reply 'yes' or 'no' before 22:12", SmsContent.booking_enquiry(booking)
  end
  ...
  test "booking enquiry with a non-integer cost" do
    ...
    SmsContent.stubs(:booking_reply_deadline).returns '22:12'
    assert_equal "VarsityCleaners: job offer in E1W. 22 March 12:00 - 16:00 (cleaning stuff provided). Will pay 45.20 pounds. Accept? Reply 'yes' or 'no' before 22:12", SmsContent.booking_enquiry(booking)
  end
